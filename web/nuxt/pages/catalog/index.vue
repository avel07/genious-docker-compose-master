...
const [{ data: catalogContent }, { data: catalog, error, refresh: refreshCatalog }] = await Promise.all([
    // Описание страницы
    useFetchApi('/api/v1/content/catalog', {
    ...
    useFetchApi(`/api/v1/catalog`, {
        ...
        key: 'api-catalog', // Кеш для гидрации
        transform: (response: any) => ({
...
// Meta head
useHead({
    title: catalogContent.value?.meta?.metaTitle ?? catalogContent.value?.title,
    meta: [
        {
            name: 'description',
            content: catalogContent.value?.meta?.metaDescription ?? catalogContent.value?.title
        }
    ]
});
</script>
...
<template>
    <main class="page">
        <div class="content">
            <Breadcrumbs currentTitle="Каталог" />
            <h1 class="fs-32 uppercase mt-16 mt-t-0 fs-m-21 fw-medium">
                {{ catalogContent?.meta?.metaPageTitle ?? catalogContent?.title ?? 'Каталог' }}
            </h1>
            ...
            <div class="section-pt" v-if="catalog?.pagination">
